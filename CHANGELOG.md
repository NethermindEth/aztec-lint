...
- Fixed `NOIR100` to avoid signature-context bleed by ending statement sensitivity scans at top-level `{}` boundaries, reducing false positives for numeric literals in function signatures.
- Fixed `NOIR100` to ignore `[T; N]` array-length literals in type-annotation positions (`:`/`->`/`as`), reducing noise for type-level arity markers such as return/log tuple sizes.
