...
const INTRODUCED_IN_V0_5_0: &str = "0.5.0";
const INTRODUCED_IN_V0_6_0: &str = "0.6.0";
...
const ALL_LINT_SPECS: &[LintSpec] = &[
    ...
    LintSpec {
        ...
        category: LintCategory::Soundness,
        maturity: LintMaturityTier::Preview,
        introduced_in: INTRODUCED_IN_V0_5_0,
        default_level: RuleLevel::Deny,
        confidence: Confidence::High,
        lifecycle: LintLifecycleState::Active,
        docs: LintDocs {
            summary: "Note consumption without nullifier emission.",
            what_it_does: "Reports note pop/consume patterns when the same function does not emit a nullifier.",
            why_this_matters: "Consumed notes without nullifiers can enable replay or double-spend style state inconsistencies.",
            known_limitations: "Function-local matching does not prove path-complete nullifier coverage in highly dynamic control flow.",
            how_to_fix: "Emit nullifiers for consumed notes or switch to helper APIs that enforce consume-and-nullify semantics.",
            examples: &[
    ...
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Flags nullifier hash call sites where required domain components are not present in hash inputs.",
            why_this_matters: "Weak nullifier domain separation can cause collisions across domains or protocol contexts.",
            known_limitations: "Heuristic token matching may miss custom domain-separation helpers or aliases.",
            how_to_fix: "Include configured domain fields (for example contract address and nonce) in nullifier hash inputs.",
            examples: &[
    ...
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Detects commitment-style hash sinks that do not include configured domain-separation components.",
            why_this_matters: "Insufficient commitment domain separation can blur security boundaries and weaken protocol assumptions.",
            known_limitations: "Rule matching focuses on recognizable commitment sink names and hash-shaped inputs.",
            how_to_fix: "Add required context fields (such as contract address and note type) to commitment hash construction.",
            examples: &[
    ...
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Reports public entrypoints that appear to mutate private note/state transitions and lack only-self protection.",
            why_this_matters: "Publicly callable private-state mutation surfaces can break intended access boundaries.",
            known_limitations: "Detection relies on recognized mutation patterns and may not cover every custom state transition helper.",
            how_to_fix: "Add `#[only_self]` to the public entrypoint or refactor the mutation into a safer private flow.",
            examples: &[
                "Mark public state-transition bridges with `#[only_self]` before calling note mutation APIs.",
            ],
            references: &[DOCS_REFERENCE_RULE_AUTHORING, DOCS_REFERENCE_DECISION_0001],
        },
    },
    LintSpec {
        id: "AZTEC034",
        ...
        docs: LintDocs {
            ...
            what_it_does: "Finds hash inputs that are cast or converted to Field without an earlier range-style constraint.",
            why_this_matters: "Missing range proofs can make hashed representations ambiguous for bounded integer semantics.",
            known_limitations: "Nearby helper-based constraints may not be recognized when they do not resemble explicit range checks.",
            how_to_fix: "Constrain numeric width before Field conversion and hashing, then keep the guarded value flow explicit.",
            examples: &["Assert bounded `amount` before hashing `amount as Field`."],
            references: &[DOCS_REFERENCE_RULE_AUTHORING, DOCS_REFERENCE_DECISION_0003],
        },
    },
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Flags `.at(x).at(x)`-style nested key repetition that often indicates copy-paste key mistakes.",
            why_this_matters: "Repeating nested map keys unintentionally can corrupt indexing logic and authorization behavior.",
            known_limitations: "Some intentionally duplicated keying patterns may require suppression when semantically correct.",
            how_to_fix: "Use distinct key expressions for each nested `.at(...)` level or extract named key variables for clarity.",
            examples: &[
                "Replace `.at(owner).at(owner)` with the intended second key such as `.at(owner).at(spender)`.",
            ],
            references: &[DOCS_REFERENCE_RULE_AUTHORING, DOCS_REFERENCE_SUPPRESSION],
        },
    },
    LintSpec {
        id: "AZTEC036",
        ...
        category: LintCategory::Privacy,
        maturity: LintMaturityTier::Preview,
        introduced_in: INTRODUCED_IN_V0_6_0,
        default_level: RuleLevel::Warn,
        confidence: Confidence::Medium,
        lifecycle: LintLifecycleState::Active,
        docs: LintDocs {
            summary: "Secret-dependent branch affects enqueue behavior.",
            what_it_does: "Flags private or secret-influenced branching that changes whether or how enqueue-style bridge calls are emitted.",
            why_this_matters: "Observer-visible enqueue shape differences can leak private branch decisions.",
            known_limitations: "Pattern matching is currently heuristic and may not cover every custom enqueue wrapper.",
            how_to_fix: "Refactor enqueue behavior so public bridge decisions are independent of secret branch predicates.",
            examples: &[
    ...
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Reports branch-dependent behavior where secret inputs influence the number or presence of delivery-style effects.",
            why_this_matters: "Varying delivery cardinality on secret predicates can reveal private state through externally visible behavior.",
            known_limitations: "Delivery sink coverage is currently scoped to recognized call patterns.",
            how_to_fix: "Keep delivery count and emission structure invariant with respect to secret branch conditions.",
            examples: &["Avoid conditional delivery emission based on private note values."],
            references: &[DOCS_REFERENCE_RULE_AUTHORING, DOCS_REFERENCE_DECISION_0003],
        },
    },
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Detects change-note construction patterns that appear to reuse deterministic randomness or omit freshness inputs.",
            why_this_matters: "Weak randomness freshness can increase linkage risk and break expected note uniqueness properties.",
            known_limitations: "Freshness detection is heuristic and may miss user-defined entropy helper conventions.",
            how_to_fix: "Derive change-note randomness from a fresh, non-reused source and thread it explicitly into note construction.",
            examples: &[
    ...
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Flags partial-spend arithmetic patterns that do not clearly reconcile consumed, spent, and change values.",
            why_this_matters: "Unbalanced partial-spend accounting can cause invalid state transitions or silent value drift.",
            known_limitations: "Equivalent arithmetic forms may not all be recognized by pattern-driven detection.",
            how_to_fix: "Make spend and change reconciliation explicit and assert conservation-style invariants near the transition point.",
            examples: &[
    ...
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Reports initializer functions that are not protected by the expected only-self access restriction.",
            why_this_matters: "Unrestricted initializers can allow unauthorized setup flows or protocol-state takeover.",
            known_limitations: "Framework-equivalent guards not expressed through the configured only-self signal may need suppression.",
            how_to_fix: "Annotate initializer entrypoints with `#[only_self]` or move privileged initialization behind a self-only gate.",
            examples: &[
    ...
    LintSpec {
        ...
        docs: LintDocs {
            ...
            what_it_does: "Finds cast patterns between Field and bounded integers that lack nearby guard conditions proving safe range.",
            why_this_matters: "Unchecked narrowing conversions can silently corrupt values and invalidate downstream protocol logic.",
            known_limitations: "Guard recognition focuses on known range-check idioms and may miss custom helper abstractions.",
            how_to_fix: "Add explicit range checks before narrowing casts and keep the guarded value flow local and visible.",
            examples: &[
